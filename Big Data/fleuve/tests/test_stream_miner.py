...
#SET TASK
task = {"cluster-9e14-w-0": {"models": [linear_model.LogisticRegression(optim.SGD(lr=0.1))], "scaler": preprocessing.StandardScaler()}, 
        "cluster-9e14-w-1" : {"models": [dummy.NoChangeClassifier()], "scaler": preprocessing.StandardScaler()},
       "eval_metrics": [metrics.Accuracy(), utils.TimeRolling(metrics.Accuracy(), period=dt.timedelta(seconds=10)), utils.Rolling(metrics.Accuracy(), 2000)], 'workers': ["cluster-9e14-w-0", "cluster-9e14-w-1"]}
...
sm = StreamMinerMaster(data_cols, task)

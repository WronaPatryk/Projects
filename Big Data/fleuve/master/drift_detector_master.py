...
class DriftDetectorMaster(object):
    ...
    def getOutputSchema(self, task, workers):
        ...
        output_schema_ = [
            ...
            StructField("detector_name", StringType(), False),
            StructField("prediction", DoubleType(), False)
        ]

        n_models = np.max([len(models_lists) for models_lists in [model_list for model_list in [task[worker]['detectors'] for worker in workers]]])

        if  n_models > 1:
            for i in range(2, n_models + 1):
                output_schema_.append(StructField("detector_name_" + str(i), StringType(), False))
    ...
    def __init__(self, data_cols : list, task : dict, label_col="Class",workers=["cluster-9e14-w-0", "cluster-9e14-w-1"]):
        ...
        import drift_detector_handler
        f_ = udf(drift_detector_handler.DriftDetectorHandler(task), output_schema)
